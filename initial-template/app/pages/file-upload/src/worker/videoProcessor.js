export default class VideoProcessor {
	...
	renderDecodedFramesAndGetEncodedChunks(renderFrame) {
		...
		return new TransformStream({
			...
			async transform(encodedChunk, controller) {
				...
				// Precisa da versão do encode para usar webM
	...
	upload(filename, resolution, type) {
		...
		const triggerUpload = async (chunks) => {
			...
			// Realizar o upload;
			await this.#service.uploadFileToWebServer({
		...
		return new WritableStream({
			...
			async write({ data }) {
				...
				// Verifica se o tamanho de bytes for menor que 10mb. Caso não faz o upload;
	...
	async start({ file, encoderConfig, renderFrame, sendMessage }) {
		...
		await this.mp4Decoder(encoderConfig, stream)
			...
			// .pipeThrough(
			// 	new TransformStream({
			// 		transform: ({ data, position }, controlleer) => {
			// 			this.#buffers.push(data);
			// 		},
			// 		flush: () => {
			// 			// Send this messge to download and test video
			// 			// sendMessage({
			// 			// 	status: "done",
